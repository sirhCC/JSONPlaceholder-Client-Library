...
export class DeveloperTools {
  ...
  private requestInspections: Map<string, RequestInspection> = new Map();
  private responseInspections: Map<string, ResponseInspection> = new Map();
  ...
  private setupDeveloperConsole(): void {
    if (typeof window !== 'undefined') {
      ...
      (window as typeof window & { JsonPlaceholderDevTools?: unknown }).JsonPlaceholderDevTools = {
        ...
        simulateNetworkConditions: (config: NetworkSimulationConfig) => this.simulateNetworkConditions(config),
        generateExample: (operation: string) => this.generateCodeExample(operation)
      };
  ...
  inspectRequest(request: RequestInspection): void {
    if (!this.config.enabled || !this.config.requestInspection) return;
  ...
  inspectResponse(response: ResponseInspection): void {
    if (!this.config.enabled || !this.config.responseInspection) return;
  ...
  addPerformanceWarning(warning: PerformanceWarning): void {
    if (!this.config.enabled || !this.config.performanceWarnings) return;
...
const comments = await client.getComments({ postId: 1 });`,
  ...
  generateCodeExample(operation: string): CodeExample | null {
    const templates: Record<string, Partial<CodeExample>> = {
      'getPosts': {
        title: 'Fetch Posts',
